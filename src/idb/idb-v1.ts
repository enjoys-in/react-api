...
class AirsendDB<Tables> extends Dexie {

    constructor(name: string, schema: { [K in keyof Tables]: string }) {
        super(name);
    ...
    async has<T extends keyof Tables, Key extends PrimaryKeyType<Tables, T>>(
        ...
        return (await this.table(table as string).get(key as any)) !== undefined;
    }
    /**
    ...
    private getTable<K extends keyof Tables>(tableName: K) {
        return this.table(tableName as string);
    }
